    ...
    m_xMax(0.0f)
{
   ...
   unsigned long long int nVolX = ceil((maxVol[0]-minVol[0])/m_xLen);
   unsigned long long int nVolY = ceil((maxVol[1]-minVol[1])/m_yLen);
   ...
   {
      unsigned long long int rB = rV + offsetY;
      unsigned long long int bilFileIndex = (rB*m_nColumns+offsetX)*sizeof(float);
...
void DtmBilReader::readHeader(const std::string &i_dtm)
{
   ...
   m_xMax = m_xMin+m_nColumns*m_xLen;
   std::ifstream in;
